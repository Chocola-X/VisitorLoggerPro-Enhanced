...
修复的问题有：
 ...
 - 修复了当cdn分发JS等待时间过长却不切换本地文件的问题。设置了等待cdn分发静态文件超时时间为2秒，超时后自动获取本地文件。

当前版本已完成数据库、统计接口与 Typecho 1.3 后台兼容性重构，支持 MySQL 和 SQLite。

插件目录必须保持为 **VisitorLoggerPro**。

### v2.4.1 地区统计修复

- 将“国家=中国、省份=中国”的不完整定位视为省份缺失，不再把“中国”计入国内省份统计
- 数据库构建、访问记录规范化和省份查询均排除这一无效组合

### v2.4.0 IP 数据库改进

- 新增精简 IP 信息数据库，只保留国家、省份、城市、运营商四项
- 纯真数据库已转换为预先结构化的只读库，不再在访问请求中猜拆地址或强制添加“省”后缀
- IPv4 数据库可选择 ip2region、精简 IP 信息库或整理后的纯真库
- 访客日志新增 `isp` 字段，地点统一按“国家 省份 城市 运营商”显示，空值自动省略


### v2.3.1 主要改进

- 兼容 Typecho 1.2.1 与 1.3.0 的后台布局和数据库查询构造器
- 访客日志与趋势分析合并为一个控制台菜单入口
- 支持 SQLite，并修复国家、省份、城市统计查询
- 可配置 ECharts 使用公共 CDN（失败回落本地）或直接使用本地资源
- 支持按保留天数自动清理历史访问数据，默认 90 天且可关闭

### v2.3.0 主要改进

- 支持 SQLite 部署，并保留 MySQL 兼容性
- 国家、省份、城市独立存储，自动迁移旧格式地点数据
- 统计接口改为 Typecho Action，修复越权、SQL 注入、XSS 与 CSRF 风险
- 新增查询索引、定位缓存和单查询会话计算
- 自动清理超过指定天数的数据，默认保留 90 天，也可关闭
- 修复 Typecho 1.3 后台菜单、资源路径和移动端布局
...
5. 在 Typecho 后台启用插件
6. 如需使用主题前台统计模板，可按主题说明集成 `visitor-stats.php`
