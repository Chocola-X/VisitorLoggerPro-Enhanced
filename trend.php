...
// Typecho 1.2 uses the current request URL for an active extension menu item.
// Keep the sole control-panel entry pointing back to the visitor log view.
?>
<script>
document.querySelectorAll('.typecho-head-nav a[href*="panel=VisitorLoggerPro"]').forEach(function (link) {
    link.href = <?php echo json_encode($logsPanelUrl, JSON_UNESCAPED_SLASHES); ?>;
});
</script>
<?php
